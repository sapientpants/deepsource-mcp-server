...
/**
 * Manages pagination for fetching multiple pages of data
 * @public
 */
export class PaginationManager {
   ...
   */
  static async fetchMultiplePages<T>(
   ...
   */
  static addPaginationMetadata<T>(
   ...
   */
  static processPaginationParams(params: PaginationParams): {
   ...
   */
  static createPaginationIterator<T>(fetcher: PageFetcher<T>, pageSize = 50): AsyncIterable<T[]> {
    return {
   ...
   */
  static isValidCursor(cursor: string | undefined): boolean {
    if (cursor === undefined) {
    ...
    // Additional validation could be added here based on cursor format
    // For now, we assume all non-empty strings are valid cursors
    return true;
  }
   ...
   */
  static mergeResponses<T>(responses: PaginatedResponse<T>[]): PaginatedResponse<T> {
