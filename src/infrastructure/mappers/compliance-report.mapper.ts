...
/**
 * Maps between API models and domain models for ComplianceReport
 */
export class ComplianceReportMapper {
  /**
  ...
  static toDomain(
    ...
    const categories: ComplianceCategory[] = apiReport.securityIssueStats.map((stat) => {
      ...
      return {
        ...
        severity: this.mapHighestSeverity(stat.occurrence),
      };
    });
  ...
  /**
   * Maps API report status to domain status
   *
   * @param apiStatus - The API report status
   * @returns The domain compliance report status
   */
  static mapStatus(apiStatus?: string): ComplianceReportStatus {
  ...
  /**
   * Determines the highest severity level from occurrence data
   *
   * @param occurrence - The severity distribution
   * @returns The highest severity level present
   */
  static mapHighestSeverity(occurrence: {
    critical: number;
    major: number;
    minor: number;
    total: number;
   ...
   */
  static toPersistence(report: ComplianceReport): {
    id: string;
    projectKey: string;
    repositoryId: string;
   ...
   *
   * @param apiReports - Array of API compliance report models
   * @param projectKey - The project key
   * @param repositoryId - The repository GraphQL ID
  ...
  static toDomainFromList(
    ...
    return apiReports.map((report) => this.toDomain(report, projectKey, repositoryId));
  }
   ...
   */
  static estimateComplianceScore(categories: ComplianceCategory[]): number {
